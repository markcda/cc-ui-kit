...
pub fn StatusText<'a>(
  cx: Scope<'a>,
  notification_text: impl Into<HValue<'a, String>>,
) -> Element<'a> {
  let coloring_rules = if let Some(coloring) = use_shared_state::<CCUiKitRules>(cx)
    && let Some(status_text_rules) = coloring.read().additional_styles.get("status_text")
  {
    status_text_rules.to_owned()
  } else {
    "decoration-[#607a79] text-[#607a79] underline italic".into()
  };

  cx.render({
    let text = if let Some(notification_text) = notification_text.into().read_ref() {
